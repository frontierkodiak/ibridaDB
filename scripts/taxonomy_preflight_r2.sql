-- IBRIDA-008: Taxonomy Preflight Check for r2
-- Compare r1 taxa with r2 staging taxa for BREAKING changes

\timing on

-- Step 1: Find the cutoff date from r1 observations
\echo 'Step 1: Finding r1 cutoff date...'
...
-- Step 2: Identify NEW observations in r2 (observed after cutoff)
...
FULL OUTER JOIN 
    (SELECT * FROM stg_inat_20250827.taxa WHERE taxon_id IN (SELECT taxon_id FROM r2_new_taxa)) r2
...
\echo ''
\echo '=== TAXONOMY PREFLIGHT RESULTS ==='
\echo ''
...
\echo ''
\echo 'BREAKING changes detail (if any):'
...
BEGIN
    ...
    IF breaking_count > 0 THEN
        RAISE EXCEPTION 'PREFLIGHT FAILED: Found % BREAKING taxonomy changes. Pipeline blocked!', breaking_count;
    ELSE
        RAISE NOTICE 'PREFLIGHT PASSED: No breaking taxonomy changes detected. Safe to proceed with r2 import.';
    END IF;
END $$;